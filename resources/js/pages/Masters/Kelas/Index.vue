...
const requestDelete = async (id) => {
    return await http.delete(`/kelas/${id}`)
}
...
const reset_req = async () => {
    return await http.post('kelas/reset')
}
...
<template>
    <div class="p-3">
        ...
        <div class="flex lg:flex-row items-start gap-3 flex-wrap">
            <router-link class="btn btn-neutral btn-sm" :to="{ name: 'create-kelas' }">
        ...
        <Datatable url="kelas" :columns="columns" :update="update">
            ...
            <template #content="data">
                ...
                <td>
                    {{ data.data.sesi_count }}
                </td>
                <th class="flex flex-wrap gap-2">
                    <router-link :to="{ name: 'edit-kelas', params: { id: data.data.id } }"
